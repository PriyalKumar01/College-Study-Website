...
import { ArrowLeft, Coins, Shield, Crown, Calendar } from "lucide-react";
...
// ─── Admin Avatar ─────────────────────────────────────────────────────────────
function AdminAvatar({ name, email, size = 44 }: { name: string; email: string; size?: number }) {
  const initials = getInitials(name);
  const grad = getGradient(email);
  return (
    <div
      style={{
    ...
    >
      ...
      >
        ...
        <div
          ...
          onMouseEnter={e => {
            (e.currentTarget as HTMLElement).style.boxShadow = `0 4px 20px rgba(30,27,75,0.13)`;
            (e.currentTarget as HTMLElement).style.background = isOwner
        ...
        >
          ...
          <div style={{
            ...
            display: "flex", alignItems: "center", justifyContent: "center",
            background: isOwner ? "#fef3c7" : "rgba(99,102,241,0.08)",
            fontSize: isOwner ? 18 : 13,
            fontWeight: 800,
            color: isOwner ? "#d97706" : "#6366f1",
          }}>
            {isOwner ? "👑" : index + 1}
          </div>

          {/* Avatar */}
          <AdminAvatar name={displayName} email={admin.user_email} size={46} />

          {/* Info */}
          <div style={{ flex: 1, minWidth: 0 }}>
            {/* Name — with hover to sidebar purple */}
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 2 }}>
              <span
                className="admin-name-hover"
                style={{
                  fontWeight: 700,
                  fontSize: 15,
              ...
              <span style={{
                ...
                background: isOwner ? "#fef3c7" : "rgba(99,102,241,0.1)",
                color: isOwner ? "#b45309" : "#4f46e5",
                border: `1.5px solid ${isOwner ? "#fcd34d" : "rgba(99,102,241,0.3)"}`,
                letterSpacing: 0.3,
              }}>
          ...
          {(admin.from_date || admin.to_date) && (
            <div style={{ textAlign: "right", flexShrink: 0, minWidth: 90 }} className="hidden sm:block">
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 1 }}>
                <span style={{ fontSize: 12, fontWeight: 600, color: "var(--foreground)", opacity: 0.7 }}>
                  {fmtDate(admin.from_date) || "?"}
                </span>
                <span style={{ fontSize: 11, color: "var(--muted-foreground)" }}>
                  → {admin.to_date
                    ? fmtDate(admin.to_date)
...
const NotesContributors = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 relative">
      ...
      {/* Global hover CSS */}
      <style>{`
        .admin-card-inner:hover .admin-name-hover {
          color: ${SIDEBAR_PURPLE} !important;
        }
        .dark .admin-card-inner:hover .admin-name-hover {
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <div className="absolute top-40 -right-20 w-96 h-96 bg-indigo-400/10 rounded-full blur-3xl" style={{ animationDelay: "700ms" }} />
      </div>
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                border: "none",
                cursor: "pointer",
                transition: "background 0.25s cubic-bezier(.4,0,.2,1), color 0.25s",
                background: tab === "contributors"
                  ? "linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%)"
                  : "#ffffff",
                color: tab === "contributors" ? "#ffffff" : "#6366f1",
            ...
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                transition: "background 0.25s cubic-bezier(.4,0,.2,1), color 0.25s",
                background: tab === "admins"
                  ? "linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%)"
                  : "#ffffff",
                color: tab === "admins" ? "#ffffff" : "#6366f1",
                letterSpacing: 0.3,
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                        ...
                        <div className="flex items-center gap-4 md:gap-6">
                          ...
                          <div>
                            <a href={contributor.socialUrl || "#"} target="_blank" rel="noopener noreferrer" className="hover:underline decoration-slate-300 underline-offset-4">
                              <h3 className="font-bold text-lg md:text-xl text-slate-800 dark:text-slate-100 flex items-center gap-1.5 hover:text-primary transition-colors">
                                {contributor.name}
                              </h3>
                            </a>
            ...
            >
              ...
              {loadingAdmins ? (
                <div className="flex flex-col items-center justify-center py-20 gap-4">
                  ...
                  <p className="text-sm text-muted-foreground">Loading admin team…</p>
                </div>
              ) : (
                <div className="max-w-3xl mx-auto">
                  {admins.length === 0 ? (
                    ...
                    admins.map((admin, i) => (
                      <AdminCard key={admin.id} admin={admin} index={i} />
                    ))
                  )}
                </div>
              )}
