...
const SixthSemesterCENotes = () => {
  ...
  const handleWhatsAppShare = (subjectName: string, subjectId: string) => {
    ...
    const message = `Check out ${subjectName} notes for 6th Semester CE on College Study Hub: ${shareUrl}`;
  ...
  const staticSubjects = [
    {
      ...
      icon: '🏗️',
      color: 'bg-gray-600',
      notes: [{ title: 'RCD Complete Notes', url: '#' }],
    },
    {
      id: 'geotech',
      ...
      icon: '⛏️',
      color: 'bg-yellow-700',
      notes: [{ title: 'Geotechnical Engineering Notes', url: '#' }],
    },
    {
      id: 'water',
      ...
      icon: '💧',
      color: 'bg-blue-500',
      notes: [{ title: 'Water Resources Notes', url: '#' }],
    },
    {
      id: 'transport',
      ...
      icon: '🛣️',
      color: 'bg-orange-500',
      notes: [{ title: 'Transportation Engineering Notes', url: '#' }],
    },
    {
      id: 'env',
  ...
  const { data: communityNotes, refetch: refreshNotes } = useCommunityNotes('btech', 'CE-6th Semester');
  const subjects = staticSubjects.map((sub) => ({
    ...
    notes: [
      ...
      ...(communityNotes || [])
        ...
        .map((cn) => ({
          id: cn.id, title: cn.title, url: cn.file_url,
          isCommunity: true, fileName: cn.file_name,
          uploadedBy: cn.uploaded_by, userName: cn.user_name,
        })),
    ],
  }));
      ...
      <div className="min-h-screen bg-gradient-hero">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-8">
            ...
            <p className="text-muted-foreground text-lg">All notes for {subject.name} - 6th Semester CE</p>
          </motion.div>
                  ...
                  <CardContent>
                    <div className="flex gap-2">
                      ...
                      {(note as any).isCommunity && isOwner && (
                        <Button variant="destructive" size="icon" onClick={() => handleDeleteCommunityNote((note as any).id)}><Trash2 className="h-4 w-4" /></Button>
                      )}
                    </div>
                  </CardContent>
                </Card>
    ...
    <div className="min-h-screen bg-gradient-hero">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-2">6th Semester CE Notes 🏗️</h1>
          <p className="text-muted-foreground text-lg"><strong>Civil Engineering</strong> — 6th Semester</p>
        </motion.div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <Card className="feature-card h-full cursor-pointer transition-all duration-300 border-2 border-transparent hover:border-primary/20 shadow-lg hover:shadow-xl" onClick={() => setSelectedSubject(subject.id)}>
                <CardHeader>
                  <div className="flex justify-end mb-2">
                    <Button variant="ghost" size="icon" className="text-green-600 hover:bg-green-50 h-8 w-8 -mt-2 -mr-2" onClick={(e) => { e.stopPropagation(); handleWhatsAppShare(subject.name, subject.id); }}><Share2 className="h-4 w-4" /></Button>
                  </div>
...
export default SixthSemesterCENotes;
