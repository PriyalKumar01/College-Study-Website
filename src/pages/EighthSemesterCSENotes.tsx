...
const EighthSemesterCSENotes = () => {
  ...
  const handleWhatsAppShare = (subjectName: string, subjectId: string) => {
    ...
    const message = `Check out ${subjectName} notes for 8th Semester CSE on College Study Hub: ${shareUrl}`;
  ...
  const syllabus = {
    title: '8th Semester CSE Syllabus',
    url: '#'
  };

  const { data: communityNotes, refetch: refreshNotes } = useCommunityNotes('btech', ['CSE-8th Semester', 'IT-8th Semester', 'CSE-AIML-8th Semester']);
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="bg-foreground dark:bg-card text-background dark:text-foreground pt-16 pb-10 px-4 sm:px-8">
          <div className="max-w-5xl mx-auto">
            ...
            <p className="opacity-60 text-sm">{subject.notes.length} file{subject.notes.length !== 1 ? 's' : ''} available · 8th Semester B.Tech</p>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-foreground dark:bg-card text-background dark:text-foreground pt-16 pb-12 px-4 sm:px-8">
        <div className="max-w-5xl mx-auto">
          <button onClick={() => navigate('/btech-notes/fourth-year/semester-8')} className="inline-flex items-center gap-2 text-xs font-semibold tracking-widest uppercase opacity-50 hover:opacity-100 transition-opacity mb-8">
            <ArrowLeft className="h-3.5 w-3.5" /> Back to Branches
          </button>
          <p className="text-xs font-bold tracking-[0.2em] uppercase opacity-50 mb-3">8th Semester — B.Tech Notes</p>
          ...
          <p className="text-sm opacity-60 mb-8 max-w-xl">Comprehensive study materials, notes, playlists, and PYQs for CSE, IT and CSE (AI & ML) 8th Semester.</p>
          <div className="flex flex-wrap gap-2">
            ...
            <span className="text-xs font-semibold tracking-wider uppercase border border-background/30 px-3 py-1.5 rounded">8th Semester</span>
          </div>
        </div>
      </div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.4 }} className="border border-border rounded-xl p-5 bg-card flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Official syllabus for 8th semester CSE</p>
            </div>
          </div>
          <button onClick={() => handleDownload(syllabus.url, syllabus.title)} className="inline-flex items-center gap-2 text-xs font-bold tracking-wider uppercase py-2.5 px-5 rounded-lg bg-foreground text-background hover:opacity-80 transition-opacity" disabled={syllabus.url === '#'}>
