...
const FourthSemesterCENotes = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { user, isOwner } = useAuth();
  const { toast } = useToast();

  const { data: communityNotes, refetch: refreshNotes } = useCommunityNotes('btech', 'CE-4th Semester');
  ...
  const subjectPlaylists: Record<string, { detailed: { title: string; url: string; recommended?: boolean }[]; oneshot: { title: string; url: string; recommended?: boolean }[] }> = {
    ...
    sm: {
      detailed: [
        { title: 'Strength of Materials Complete', url: '#', recommended: true },
      ],
      oneshot: []
    },
    survey: {
      detailed: [
        { title: 'Surveying Complete', url: '#', recommended: true },
      ],
      oneshot: []
    }
  };
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="bg-foreground dark:bg-card text-background dark:text-foreground pt-16 pb-10 px-4 sm:px-8">
          <div className="max-w-5xl mx-auto">
            ...
            <p className="text-xs opacity-50 uppercase tracking-widest">Civil Engineering — 4th Semester</p>
          </div>
        </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Official syllabus for 4th semester B.Tech CE</p>
            </div>
          </div>
          <button
...
export default FourthSemesterCENotes;
