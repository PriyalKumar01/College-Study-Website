...
const FifthSemesterLFTNotes = () => {
  ...
  const handleWhatsAppShare = (subjectName: string) => {
    ...
    const message = `Check out ${subjectName} notes for 5th Semester LFT on College Study Hub: ${shareUrl}`;
    window.open(`https://wa.me/?text=${encodeURIComponent(message)}`, '_blank');
  };

  // Subject names MUST match courseStructure.ts 'LFT-5th Semester' entries EXACTLY
  // courseStructure: Leather Finishing, Quality Control, Leather Goods, Environmental Management
  const staticSubjects = [
    {
      id: 'leather-finishing',
      name: 'Leather Finishing',
      fullName: 'Leather Finishing',
      icon: '👞',
      color: 'bg-amber-700',
    ...
    },
    {
      ...
      fullName: 'Quality Control in Leather',
      icon: '✅',
      color: 'bg-green-600',
      playlists: { detailed: [], oneshot: [] },
      notes: []
    },
    {
      id: 'leather-goods',
      name: 'Leather Goods',
      fullName: 'Leather Goods Manufacturing',
      icon: '👜',
      color: 'bg-brown-600',
      playlists: { detailed: [], oneshot: [] },
      notes: []
  ...
  // Inject community notes into each subject by matching subject name exactly
  const subjects: any[] = staticSubjects.map(sub => ({
    ...sub,
    notes: [
      ...sub.notes,
      ...(communityNotes || [])
        .filter(cn => cn.subject === sub.name || cn.subject === sub.fullName)
        .map(cn => ({
          id: cn.id,
          title: cn.title,
          url: cn.file_url,
    ...
    return (
      <div className="min-h-screen bg-gradient-hero">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <h1 className="text-3xl font-bold text-foreground mb-6">{subject.fullName} 📚</h1>
          {subject.notes.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-muted-foreground text-lg">No notes uploaded yet for this subject.</p>
                    ...
                    <CardHeader>
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div className="flex gap-1">
                          ...
                          {note.isCommunity && <Badge variant="outline" className="text-xs border-green-500 text-green-600">Community</Badge>}
                        </div>
                      </div>
    ...
    <div className="min-h-screen bg-gradient-hero">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-foreground">5th Semester LFT Notes</h1>
          <p className="text-lg text-muted-foreground">Leather &amp; Fashion Technology study materials</p>
        </motion.div>
              ...
              <motion.div key={subject.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
                <Card className="feature-card h-full relative">
                  ...
                  <CardHeader>
                    ...
                    <CardDescription>
                      {subject.notes.length > 0 ? `${subject.notes.length} PDF Notes Available` : 'No notes yet — upload to be first!'}
                    </CardDescription>
                  </CardHeader>
...
export default FifthSemesterLFTNotes;
