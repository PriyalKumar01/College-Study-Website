...
const SixthSemesterBTNotes = () => {
  const navigate = useNavigate();
  const { user, isOwner } = useAuth();
  const { toast } = useToast();
  const [selectedSubject, setSelectedSubject] = useState<string | null>(null);

  const { data: communityNotes, refetch: refreshNotes } = useCommunityNotes('btech', 'BT-6th Semester');
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-foreground dark:bg-card text-background dark:text-foreground pt-16 pb-12 px-4 sm:px-8">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-sm opacity-50 mb-8">B.Tech. Biotechnology — 6th Semester resources</p>
        </div>
      </div>
