...
const FourthSemesterLFTNotes = () => {
  ...
  const handleWhatsAppShare = (subjectName: string) => {
    ...
    const message = `Check out ${subjectName} notes for 4th Semester LFT on College Study Hub: ${shareUrl}`;
  ...
  const subjectPlaylists: Record<string, { detailed: { title: string; url: string; recommended?: boolean }[]; oneshot: { title: string; url: string; recommended?: boolean }[] }> = {
    ...
    lp: {
      detailed: [
        { title: 'Leather Processing Complete', url: '#', recommended: true },
      ],
      oneshot: []
    }
  };
    ...
    <div className="min-h-screen bg-gradient-hero">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-foreground">4th Semester LFT Notes</h1>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Leather & Fashion Technology study materials</p>
        </motion.div>
