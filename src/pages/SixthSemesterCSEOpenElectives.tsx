...
const SixthSemesterCSEOpenElectives = () => {
  ...
  const handleWhatsAppShare = (subjectName: string, subjectId: string) => {
    ...
    const message = `Check out ${subjectName} notes for 6th Semester Open Elective on College Study Hub: ${shareUrl}`;
    ...
    return (
      <div className="min-h-screen bg-gradient-hero">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-6">{subject.name} 📚</h1>
  ...
  return (
    <div className="min-h-screen bg-gradient-hero">
      <Navbar />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Button onClick={() => navigate('/btech-notes/third-year/semester-6/cse')} variant="outline" className="mb-4">
          <ArrowLeft className="h-4 w-4 mr-2" /> Back to 6th Sem CSE
        </Button>
        <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-2">6th Semester Open Electives 📚</h1>
        <p className="text-muted-foreground text-lg mb-8">Choose from available open elective subjects</p>
              ...
              <Card className="feature-card h-full">
                <CardHeader>
                  <div className="flex items-center justify-between mb-3">
                    <div className={`w-16 h-16 ${subject.color} rounded-full flex items-center justify-center text-2xl`}>{subject.icon}</div>
                    <Button variant="ghost" size="icon" className="text-green-600" onClick={() => handleWhatsAppShare(subject.name, subject.id)}>
                      <Share2 className="h-4 w-4" />
                    </Button>
                  </div>
                  <CardTitle className="text-lg text-center">{subject.name}</CardTitle>
                  <CardDescription className="text-center">{subject.notes.length} notes</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex flex-col gap-3">
                    <div className="border-t pt-4">
                      <div className="flex items-center justify-between cursor-pointer hover:bg-muted/50 rounded p-2 -m-2" onClick={() => toggleSubjectExpansion(subject.id)}>
                      ...
                      {expandedSubjects.includes(subject.id) && (
                        <div className="mt-3 space-y-2 pl-2">
                          {getSubjectPlaylists(subject.id).detailed.length > 0 && (
                            <Button variant="ghost" size="sm" className="w-full justify-start text-xs h-8" onClick={() => handlePlaylistClick(subject.id, 'detailed')}>
                              📚 Detailed Playlists ({getSubjectPlaylists(subject.id).detailed.length})
                            </Button>
                          )}
                          {getSubjectPlaylists(subject.id).oneshot.length > 0 && (
                            <Button variant="ghost" size="sm" className="w-full justify-start text-xs h-8" onClick={() => handlePlaylistClick(subject.id, 'oneshot')}>
...
export default SixthSemesterCSEOpenElectives;
